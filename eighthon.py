...
async def join_channel():
    try:
        await eighthon(JoinChannelRequest("@x92xxx"))
...
ownersaif_id = 6144471881 or 6144471881
@x92xxx.on(events.NewMessage(outgoing=False, pattern='/start'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersaif_id :
        order = await event.reply('** Hi MY Developer - @L_3_R**')
